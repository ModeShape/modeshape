...
settings_file = '%s/.modeshape_dev_settings' % os.getenv('HOME')
...
default_settings = {'dry_run': False, 'multi_threaded': False, 'verbose': False, 'use_colors': True, 'skip_tests' : False}
...
def get_settings():
  """Retrieves user-specific settings for all ModeShape tools.  Returns a dict of key/value pairs, or an empty dict if the settings file doesn't exist."""
  f = None
  try:
    settings = {}
...
def handle_release_virgin():
  """This sounds dirty!"""
  prettyprint("""
    It appears that this is the first time you are using this script.  Please answer a few questions before
    we can proceed. Press ENTER to accept the default value, which appears in brackets.
    ...
    while True:
      try:
        ...
        if fnmatch.fnmatch(file, self.pattern):
          return fullname
  def replace_all_in(self, afile, search_expr, replacement ):
    f = open(afile)
    content = f.read()
    f.close()
    s_exp = re.compile(search_expr,re.M)
    modified = False
    if s_exp.search(content):
      content = s_exp.sub(replacement,content)
      #print content
      f = open(afile,'w')
      f.write(content)
      f.close()
...
class Git(object):  
  '''Encapsulates git functionality necessary for releasing ModeShape'''
  cmd = 'git'
  ...
  def is_upstream_clone(self):
    ...
    return len(push_urls) == 1 and push_urls[0] == 'git@github.com:ModeShape/modeshape.git'
  ...
  def commit(self, files):
    ...
    self.run_git(["commit", "-m", "'Release: update versions for %s'" % self.tag])
  
  def tag_for_release(self):
    '''Tags the current branch for release using the tag name.'''
    self.run_git(["tag", "-a", "-m", "'Release: tag %s'" % self.tag, self.tag])
...
class DryRun(object):
  location_root = "%s/%s" % (os.getenv("HOME"), "modeshape_release_dry_run")
  flags = "-r"
...
def get_version_pattern(): 
  return re.compile("^([2-9]\.[0-9])\.[0-9]\.(Final|(Alpha|Beta|CR)[1-9][0-9]?)$", re.IGNORECASE)
...
def unmarkdown(markdown_text):
  # Define the dictionary of replacement regular expressions, keyed by the search expression
  expressions = {
     "\[(.*?)\]\[.*?]" : "\\1",  # replace links
     ...
     "_(.*?)_" : "\\1",          # remove emphasis
     "^\\s*\[(.*?)\]\\s*:\\s*(http|ftp|ssh).*$\\n" : "",  # replace links
  }

  # Now replace each of the regex pairs
  text = markdown_text
  for s_exp,r_exp in expressions.items():
    search_exp = re.compile(s_exp,re.M)
    text = search_exp.sub(r_exp,text)
  return text;
