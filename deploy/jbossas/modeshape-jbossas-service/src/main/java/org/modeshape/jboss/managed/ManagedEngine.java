...
public final class ManagedEngine implements ModeShapeManagedObject {
    ...
    public void setConfigURL( URL configurationUrl ) throws Exception {
        ...
        // Read the configuration here (we want to throw any exceptions right here) ...
    ...
    public synchronized void restart() throws Exception {
        ...
        try {
            ...
            // There was a problem starting the new engine, so keep the old engine (which is still running) ...
            this.engine = oldEngine;
            if (e instanceof RuntimeException) throw (RuntimeException)e;
            throw (Exception)e;
        }

        // At this point, we know that the new engine was started correctly and is ready to be used.
        // So now we can shutdown the old engine (preventing new sessions from being created), waiting up to 10 seconds
        // for any previously-created sessions to be closed gracefully, and then forcing termination of all
    ...
    public List<ManagedProperty> getProperties( String objectName,
        ...
        if (objectType.equals(Component.CONNECTOR)) {
            ...
            RepositoryConnectionPool connectionPool = engine.getRepositoryService()
                                                            .getRepositoryLibrary()
