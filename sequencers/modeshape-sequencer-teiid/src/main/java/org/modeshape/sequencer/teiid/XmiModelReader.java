 ...
 */
@NotThreadSafe
public class XmiModelReader extends XmiNodeReader {
    ...
    private final Map<String, PropertySet> mmuuidToPropertySet = new HashMap<String, PropertySet>();
     ...
     * 
     * @param modelParentNode the node under which the model node should be created by this reader; may not be null
     * @param modelName the name of the model file
     * @param pathToModelFile the path of the original model file
     * @param xmiRootNode the node containing the model contents
     ...
     *        VDB
     */
    protected XmiModelReader( Node modelParentNode,
                           ...
                           VdbModel vdbModel ) throws RepositoryException {
        super(xmiRootNode, context, generateShortTypeNames);

        this.defaults = DefaultProperties.getDefaults();
        this.originalFile = pathToModelFile;
        this.outputNode = modelParentNode.addNode(modelName);
    ...
    public void setResolver( ReferenceResolver resolver ) {
        this.resolver = resolver != null ? resolver : new ReferenceResolver(rootNode.getGraph().getContext());
    }
    ...
    public boolean writePhase0( SequencerOutput output ) {
        ...
        for (SubgraphNode node : rootNode) {
    ...
    public boolean writePhase1( SequencerOutput output ) {
        ...
        SubgraphNode xmi = rootNode.getRoot();
        ...
        if (annotationContainer != null) {
            ...
            for (Location objectLocation : annotationContainer.getChildren()) {
                SubgraphNode modelObject = rootNode.getNode(objectLocation);
                processObject(outputNode, modelObject, output);
            }
        }
        ...
        if (modelAnnotation != null) {
            ...
            resolver.recordXmiUuid(xmiUuid, outputNode);
            ...
            props.writeTo(output, outputNode);
            output.setProperty(outputNode, JcrLexicon.UUID, uuidFor(modelAnnotation));
            output.setProperty(outputNode, XmiLexicon.UUID, xmiUuid);
        } else {
            // Create the root node for this XMI model ...
            output.setProperty(outputNode, JcrLexicon.PRIMARY_TYPE, primaryType);
            output.setProperty(outputNode,
                               JcrLexicon.MIXIN_TYPES,
                               CoreLexicon.MODEL,
                               JcrMixLexicon.REFERENCEABLE,
                               XmiLexicon.REFERENCEABLE);

            output.setProperty(outputNode, XmiLexicon.VERSION, firstValue(xmi, "xmi:version", 2.0));
        }

        if (originalFile != null) {
            output.setProperty(outputNode, CoreLexicon.ORIGINAL_FILE, originalFile);
        }
        if (sha1 != null) {
            output.setProperty(outputNode, ModeShapeLexicon.SHA1, sha1);
        }

        // Write out the VDB-related information (if applicable) ...
        if (vdbModel != null) {
            output.setProperty(outputNode, VdbLexicon.VISIBLE, vdbModel.isVisible());
            output.setProperty(outputNode, VdbLexicon.CHECKSUM, vdbModel.getChecksum());
            output.setProperty(outputNode, VdbLexicon.BUILT_IN, vdbModel.isBuiltIn());
            output.setProperty(outputNode, VdbLexicon.PATH_IN_VDB, vdbModel.getPathInVdb());
            ...
            if (translator != null) output.setProperty(outputNode, VdbLexicon.SOURCE_TRANSLATOR, translator);
            if (sourceName != null) output.setProperty(outputNode, VdbLexicon.SOURCE_NAME, sourceName);
            if (jndiName != null) output.setProperty(outputNode, VdbLexicon.SOURCE_JNDI_NAME, jndiName);
            if (!vdbModel.getProblems().isEmpty()) {
                Path markersPath = path(outputNode, VdbLexicon.MARKERS);
        ...
        if (modelAnnotation != null) {
            ...
            for (Location modelImportLocation : modelAnnotation.getChildren()) {
                SubgraphNode modelImport = rootNode.getNode(modelImportLocation);
                processObject(outputNode, modelImport, output);
            }
        }
        return true;
    }
    ...
    protected Name nameFromKey( String keyName ) {
        ...
        if (index != -1) {
            ...
            if (namespaceRegistry.getNamespaceForPrefix(prefix) != null) {
    ...
    protected Path processObject( Path parentPath,
        ...
        Path actualPath = handler.process(path, node, rootNode, this, output);
        ...
        if (actualPath != null) {
            ...
            for (Location childLocation : node.getChildren()) {
                SubgraphNode childObject = rootNode.getNode(childLocation);
    ...
    public static class PropertySet {
        ...
        private void addDefaultsForType( Collection<Object> nodeTypes ) {
            ...
            for (Object nodeType : nodeTypes) {
                ...
                for (Entry<String, Object> defaultValue : reader.defaults.getDefaultsFor(nodeTypeString).entrySet()) {
        ...
        public Path process( Path path,
            ...
            for (Location childLocation : node.getChildren()) {
                ...
                if (child.getProperty("href") != null) {
                    ...
                    Name attributeName = reader.createNameWithCurrentNamespace(
                            childLocation.getPath().getLastSegment().getName());
