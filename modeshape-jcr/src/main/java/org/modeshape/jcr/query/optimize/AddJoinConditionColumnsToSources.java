...
public class AddJoinConditionColumnsToSources implements OptimizerRule {
    ...
    public PlanNode execute( QueryContext context,
        ...
        for (PlanNode joinNode : plan.findAllAtOrBelow(Type.JOIN)) {
            ...
            if (condition instanceof EquiJoinCondition) {
                ...
                Column joinColumn1 = columnFor(equiJoinCondition.selector1Name(), equiJoinCondition.getProperty1Name());
                Column joinColumn2 = columnFor(equiJoinCondition.selector2Name(), equiJoinCondition.getProperty2Name());
    ...
    protected boolean addIfMissing( QueryContext context,
        ...
        for (Column c : columns) {
            if (c.getPropertyName().equals(column.getPropertyName()) && c.selectorName().equals(column.selectorName())) return false;
        }
        columns.add(column);
