...
public class InfinispanBinaryStore extends AbstractBinaryStore {
    ...
    public void removeValuesUnusedLongerThan( long minimumAge,
        ...
        // todo since this is a maybe long running process, try to determine unlinked chunk entries?
        // todo what about interruption check of the GC thread?
        // todo walk through all nodes is expensive. how about introducing lucene to index metadata and use a 3rd cache as storage?
    ...
    private static class UnusedMapper implements Mapper<String, Metadata, String, String> {
        ...
        public void map(String key, Metadata metadata, Collector<String, String> stringCollector) {
            long now = System.currentTimeMillis();
            if(metadata.isUnused() && now - metadata.unusedSince() > minimumAgeInMS){
