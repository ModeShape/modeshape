...
public class LocalIndexProviderTest extends AbstractIndexProviderTest {
    ...
    @Test
    public void shouldSupportQueryOffsetWithMoreThan100Nodes() throws Exception {
        ...
        for (int i = 0; i < 102; i++) {
            ...
            node1.setProperty("dateProp", Calendar.getInstance());
            Thread.sleep(10);
        }
        session.save();

        Query query1 = jcrSql2Query("SELECT * FROM [nt:testType] AS node ORDER by node.[dateProp] DESC LIMIT 1 OFFSET 0");
        validateQuery()
                .rowCount(1L)
                .useIndex("typesIndex")
        ...
        Query query2 = jcrSql2Query("SELECT * FROM [nt:testType] AS node ORDER by node.[dateProp] DESC LIMIT 1 OFFSET 50");
        validateQuery()
                .rowCount(1L)
                .useIndex("typesIndex")
        ...
        Query query3 = jcrSql2Query("SELECT * FROM [nt:testType] AS node ORDER by node.[dateProp] DESC LIMIT 1 OFFSET 100");
        validateQuery()
                .rowCount(1L)
                .useIndex("typesIndex")
