...
public class BinaryStorageTest extends SingleUseAbstractTest {
    ...
    private static final byte[] MEDIUM_STORED_BINARY = new byte[AbstractBinaryStore.MEDIUM_BUFFER_SIZE * 2];
    ...
    public void shouldStoreBinariesIntoJDBCBinaryStore() throws Exception {
        ...
        Node testRoot = jcrSession().getRootNode().addNode("testRoot");
        testRoot.setProperty("binary", session.getValueFactory().createValue(new ByteArrayInputStream(MEDIUM_STORED_BINARY)));
        jcrSession().save();
