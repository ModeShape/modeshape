...
public class DataTypeParser implements DdlConstants {
    ...
    protected DataType parseCharStringType( DdlTokenStream tokens ) throws ParsingException {
    	...
    	if( tokens.matches(DataTypes.DTYPE_VARCHAR)) {
			...
			int length = parseBracketedInteger(tokens, dataType);
			dataType.setLength(length);
    	} else if( tokens.matches(DataTypes.DTYPE_CHAR) ||
    			tokens.matches(DataTypes.DTYPE_CHARACTER)) {
    ...
    protected DataType parseMiscellaneousType( DdlTokenStream tokens ) throws ParsingException {
    	...
    	if( tokens.matches(DataTypes.DTYPE_INTERVAL) ) {
    		...
    		// CASE 2b:  SECOND [ <left paren> <interval leading field precision>  [ <comma> <interval fractional seconds precision> ] <right paren> ]
	 ...
	 */
    protected int parseInteger( DdlTokenStream tokens , DataType dataType) {
	...
	protected String consume(DdlTokenStream tokens, DataType dataType, boolean addSpacePrefix, 
			String initialStr, String... additionalStrs)
			throws ParsingException {
	 ...
	 * @throws ParsingException
	 */
	protected boolean canConsume(DdlTokenStream tokens, DataType dataType, boolean addSpacePrefix,
			String initialStr, String... additionalStrs)
			throws ParsingException {
	 ...
	 * @throws ParsingException
	 */
	protected boolean canConsumeAnyOf(DdlTokenStream tokens, DataType dataType, boolean addSpacePrefix,
			String initialStr, String... additionalStrs)
			throws ParsingException {
