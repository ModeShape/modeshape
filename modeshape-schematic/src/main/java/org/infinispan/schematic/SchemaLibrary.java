...
/**
 * A library of JSON Schema documents. Because JSON Schemas are in fact JSON documents, this library is also a
 * DocumentLibrary.
 * 
...
public interface SchemaLibrary extends DocumentLibrary {
    ...
    * 
    * @param document
    *           the document to be validated; may not be null
    * @param schemaUri
   ...
   public interface Results extends Iterable<Problem> {
      /**
       * Determine if these results contain at least one error or warning. Calling this method is equivalent to (but
       ...
       * </pre>
       * 
       * @return <code>true</code> if there is at least one error or warning, or <code>false</code> if there are no
       *         errors or warnings
       */
      boolean hasProblems();
