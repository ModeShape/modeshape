...
/**
 * A helper that locates or safely constructs and registers schematic values with a given cache. This should be the
 * <b>only</b> way {@link SchematicEntry}s are created/retrieved, to prevent concurrent creation, registration and
 * possibly overwriting of such a value within the cache.
 * 
...
public class SchematicEntryLookup {
   ...
   /**
    * Retrieves a schematic value from the given cache, stored under the given key. If a schematic value did not exist,
    * one is created and registered in an atomic fashion.
    * 
    * @param cache
    *           underlying cache
    * @param key
   ...
   public static SchematicEntry getSchematicValue(Cache<String, SchematicEntry> cache, String key) {
      return getSchematicValue(cache, key, true);
   }
    ...
    *           if true, a new atomic map is created if one doesn't exist; otherwise null is returned if the map didn't
    *           exist.
   ...
   public static SchematicEntry getSchematicValue(Cache<String, SchematicEntry> cache, String key,
      ...
      return castValue.getProxy(cache, key, cache.getAdvancedCache().getBatchContainer(), cache.getAdvancedCache()
   ...
   public static SchematicEntry getReadOnlySchematicValue(Cache<String, SchematicEntry> cache, String key) {
      SchematicEntry existingValue = getSchematicValue(cache, key, false);
