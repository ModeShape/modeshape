...
public final class I18n {
    ...
    public static void initialize( Class<?> i18nClass ) {
        ...
        if (i18nClass.isInterface()) {
            throw new IllegalArgumentException(JdbcLocalI18n.i18nClassInterface.text(i18nClass.getName()));
        }
        ...
        synchronized (i18nClass) {
            ...
            try {
                for (Field fld : i18nClass.getDeclaredFields()) {
                    ...
                    if (fld.getType() == I18n.class) {
                        ...
                        if ((fld.getModifiers() & Modifier.PUBLIC) != Modifier.PUBLIC) {
                            throw new SystemFailureException(JdbcLocalI18n.i18nFieldNotPublic.text(fld.getName(), i18nClass));
                        }

                        // Ensure field is static
                        if ((fld.getModifiers() & Modifier.STATIC) != Modifier.STATIC) {
                            throw new SystemFailureException(JdbcLocalI18n.i18nFieldNotStatic.text(fld.getName(), i18nClass));
                        }

                        // Ensure field is not final
                        if ((fld.getModifiers() & Modifier.FINAL) == Modifier.FINAL) {
                            throw new SystemFailureException(JdbcLocalI18n.i18nFieldFinal.text(fld.getName(), i18nClass));
                        }
                ...
                throw new IllegalArgumentException(JdbcLocalI18n.i18nClassNotPublic.text(i18nClass));
            }
        }
    }
    ...
    private static void localize( final Class<?> i18nClass,
        ...
        synchronized (i18nClass) {
            ...
            if (url == null) {
                ...
                if (url == null) {
                    problems.add(JdbcLocalI18n.i18nLocalizationFileNotFound.text(localizationBaseName));
                    return;
                }
            }
            // Initialize i18n map
            ...
            Properties props = new Properties() {
                ...
                public synchronized Object put( Object key,
                    ...
                    try {
                        ...
                        if (fld.getType() != I18n.class) {
                            // Invalid field type
                            finalProblems.add(JdbcLocalI18n.i18nFieldInvalidType.text(id, finalUrl, getClass().getName()));
                        } else {
                            ...
                            if (i18n.localeToTextMap.putIfAbsent(locale, text) != null) {
                                ...
                                String prevProblem = i18n.localeToProblemMap.putIfAbsent(locale,
                                                                                         JdbcLocalI18n.i18nPropertyDuplicate.text(id,
                                                                                                                               finalUrl));
                        ...
                        finalProblems.add(JdbcLocalI18n.i18nPropertyUnused.text(id, finalUrl));
                        ...
                        if (fld.getType() == I18n.class) {
                            try {
                                ...
                                if (i18n.localeToTextMap.get(locale) == null) {
                                    i18n.localeToProblemMap.put(locale, JdbcLocalI18n.i18nPropertyMissing.text(fld.getName(), url));
                                }
                            } catch (IllegalAccessException notPossible) {
                                // Would have already occurred in initialize method, but allowing for the impossible...
                                finalProblems.add(notPossible.getMessage());
    ...
    public String problem( Locale locale ) {
        ...
        problem = JdbcLocalI18n.i18nLocalizationProblems.text(i18nClass, locale);
