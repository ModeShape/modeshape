...
public class Stopwatch implements Comparable<Stopwatch> {
	 ...
	 * This method does nothing if the stopwatch is already {@link #isRunning() running}
	 * @see #isRunning()
	 */
	public void start() {
		if ( !this.isRunning() ) {
	 ...
	 * This method does nothing if the stopwatch is not currently {@link #isRunning() running}
	 * @see #isRunning()
	 */
	public void stop() {
		if ( this.isRunning() ) {
	 ...
	 * @return the total duration, or an empty duration if this stopwatch has not been used since creation or being {@link #reset() reset}
	 */
	 ...
	 * @return the average duration, or an empty duration if this stopwatch has not been used since creation or being {@link #reset() reset}
	 */
	 ...
	 * @return the median duration, or an empty duration if this stopwatch has not been used since creation or being {@link #reset() reset}
	 */
	 ...
	 * @return the total minimum, or an empty duration if this stopwatch has not been used since creation or being {@link #reset() reset}
	 */
	 ...
	 * @return the maximum duration, or an empty duration if this stopwatch has not been used since creation or being {@link #reset() reset}
	 */
	 ...
	 * @return true if {@link #getDetailedStatistics() detailed statistics} are being kept, 
	 * or false if only {@link #getSimpleStatistics() simple statistics} are being kept.
	 */
	 ...
	 * Two different kinds of histograms can be created.  The first kind is a histogram
	 * where all of the buckets are distributed normally and all have the same width.  In this
	 * case, the 'numSigmas' should be set to 0.
	 * <p>
	 * <i>Note: if only {@link #getSimpleStatistics() simple statistics} are being kept,
	 * the resulting histogram is always empty.
	 * <p>
	 * The second kind of histogram is more useful when most of the data that is clustered
	 * near one value.  This histogram is focused around the values that are up to 'numSigmas'
	 * above and below the {@link #getMedianDuration() median}, and all values outside of this range
	 * are placed in the first and last bucket.
	 * </p>
	 * @param numSigmas the number of standard deviations from the {@link #getMedianDuration() median},
	 * or 0 if the buckets of the histogram should be evenly distributed
	 * @return the histogram
	 */
	public Histogram<Duration> getHistogram( int numSigmas ) {
		return this.detailedStats != null ? this.detailedStats.getHistogram(numSigmas)
										  : new Histogram<Duration>(this.stats.getMathOperations());
	}
