...
/**
 * Test the {@link Logger} class, ensuring that it uses Log4J appropriately.
 * The {@link Logger} class uses the SLF4J generalized logging framework,
 * which can sit on top of multiple logging frameworks, including Log4J.
 * Therefore, this test assumes that SLF4J works correctly for all logging frameworks,
 * then the {@link Logger} class can be tested by using it and checking
 * the resulting Log4J output.  
 * <p>
 * To ensure that the Log4J configuration used in the remaining tests
 * (in <code>src/test/resources/log4j.properties</code>) does not interfere
 * with this test, the underlying Log4J logger is obtained before each test
 * and programmatically reconfigured and, after each test, is then restored
 * to it's previous state.  This reconfiguration involves identifying and removing all
 * of the {@link Appender Log4J Appender} on the tree of Log4J loggers,
 * and substituting a special {@link LogRecorder Appender} that records
 * the log messages in memory.  During the test, this in-memory list of
 * log messages is checked by the test case using standard assertions
 * to verify the proper order and content of the log messages.  
 * After each of the tests, all of the original
...
public class LoggerTest {
	 ...
	 * {@link #removeFirst(org.jboss.dna.common.util.Logger.Level, String, Class) validated}
	 * to ensure that the log contains messages in the proper order and with the proper content.
	 */
	public class LogRecorder extends WriterAppender {
		 ...
		 * @param expectedMessageExpression the message that the next log message should have, or a regular expression
		 * that would match the log message
